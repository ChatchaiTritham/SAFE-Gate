...
def test_gate2_cardiovascular():
    ...
    assert (
        'confidence_detail' in reason
    ), "Should include confidence detail (c2 formula)"
...
def test_gate3_data_quality():
    ...
    assert (
        len(g3.essential_fields) == 22
    ), f"Should have 22 fields, got {len(g3.essential_fields)}"
...
def test_gate4_syndrome_matching():
    ...
    assert (
        reason['best_match'] == 'BPPV'
    ), f"Best match should be BPPV, got {reason['best_match']}"
    ...
    assert tier in [
        RiskTier.R2,
        RiskTier.R3,
    ], f"Central signs should be R2-R3, got {tier}"
    ...
    print(
        "  [PASS] Gate 4: Weighted Hamming distance, 3 syndromes, thresholds 0.85/0.60"
    )
...
def test_gate5_uncertainty():
    ...
    assert g5.HIDDEN_LAYERS == [
        128,
        64,
    ], f"Hidden layers should be [128,64], got {g5.HIDDEN_LAYERS}"
    ...
    assert (
        abs(conf - expected_conf) < 0.01
    ), f"c5 should be 1-mu={expected_conf:.3f}, got {conf:.3f}"
...
def test_gate6_temporal():
    ...
    assert tier in [
        RiskTier.R2,
        RiskTier.R3,
    ], f"Acute stable should be R2-R3, got {tier}"
    ...
    assert tier in [
        RiskTier.R4,
        RiskTier.R5,
    ], f"Chronic improving should be R4-R5, got {tier}"
...
def test_acwcm_merging():
    ...
    assert tier in [
        RiskTier.R1,
        RiskTier.R2,
    ], f"CND: R1 present -> R1 or R2, got {tier}"
    ...
    assert (
        tier.value <= RiskTier.R4.value
    ), f"Bounded relaxation: should be <= R4, got {tier}"
...
def test_full_pipeline():
    ...
    assert (
        result['final_tier'] == 'R1'
    ), f"Critical case should be R1, got {result['final_tier']}"
    ...
    assert result['final_tier'] in [
        'R4',
        'R5',
    ], f"Safe case should be R4-R5, got {result['final_tier']}"
    ...
    assert (
        result['final_tier'] == 'R*'
    ), f"Incomplete should be R*, got {result['final_tier']}"
